...
/* Programa servidor */
/* Implementa la función main del servidor y aquí se registra el servicio */
...
suma_prog_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	...
	switch (rqstp->rq_proc) { //switch para todos los procedimientos remotos soportados por este programa y esta version
	case NULLPROC: //Procedimiento nulo
		(void) svc_sendreply (transp, (xdrproc_t) xdr_void, (char *)NULL);
		return;

	case SUMA: //Procedimiento SUMA
		_xdr_argument = (xdrproc_t) xdr_dupla_int;
		_xdr_result = (xdrproc_t) xdr_int;
		local = (char *(*)(char *, struct svc_req *)) suma_1_svc; //apuntador a función (local) a la función de servidor suma_1_svc
		break;
